...
{   // name        sym   color    sym_b   color_b  dmg  dur  par1 par2  item
    { "null part",  '?', c_red,     '?', c_red,     100, 100, 0, 0, itm_null, 0,
        0 },
    { "seat",       '#', c_red,     '*', c_red,     60,  300, 0, 0, itm_seat, 0,
    ...
    { "door",       '+', c_cyan,    '&', c_cyan,    80,  200, 0, 0, itm_frame, 1,
    ...
    { "blade",      '|', c_white,   'x', c_white,   350, 100, 0, 0, itm_machete, 2,
        mfb(vpf_external) | mfb(vpf_unmount_on_damage) | mfb(vpf_sharp) | mfb(vpf_no_reinforce) },
    { "spike",      '.', c_white,   'x', c_white,   300, 100, 0, 0, itm_spear_knife, 1,
        mfb(vpf_external) | mfb(vpf_unmount_on_damage) | mfb(vpf_sharp) | mfb(vpf_no_reinforce) },

//                                                           size
    { "large wheel",'0', c_dkgray,  'x', c_ltgray,  50,  300, 30, 0, itm_big_wheel, 2,
...
//                                                                         power type
    ...
    { "6L combustion engine",       '*', c_ltred,  '#', c_red,     80, 400, 800, AT_GAS, itm_combustion_large, 4,
        mfb(vpf_internal) | mfb(vpf_engine) },
    { "electric motor",             '*', c_yellow,  '#', c_red,    80, 200, 70, AT_BATT, itm_motor, 2,
        mfb(vpf_internal) | mfb(vpf_engine) },
    { "large electric motor",       '*', c_yellow,  '#', c_red,    80, 400, 350, AT_BATT, itm_motor_large, 3,
        mfb(vpf_internal) | mfb(vpf_engine) },
    { "plasma engine",              '*', c_ltblue,  '#', c_red,    80, 250, 400, AT_PLASMA, itm_plasma_engine, 5,
        mfb(vpf_internal) | mfb(vpf_engine) },
//                                                                         capacity type
    ...
    { "storage battery",            'O', c_yellow,  '#', c_red,    80, 300, 1000, AT_BATT, itm_storage_battery, 2,
        mfb(vpf_internal) | mfb(vpf_fuel_tank) },
    { "minireactor",                'O', c_ltgreen,  '#', c_red,    80, 700, 10000, AT_PLUT, itm_minireactor, 5,
        mfb(vpf_internal) | mfb(vpf_fuel_tank) },
    { "hydrogene tank",             'O', c_ltblue,  '#', c_red,     80, 150, 3000, AT_PLASMA, itm_metal_tank, 1,
        mfb(vpf_internal) | mfb(vpf_fuel_tank) },
    { "trunk",                      'H', c_brown,  '#', c_brown,    80, 300, 400, 0, itm_frame, 0,
        mfb(vpf_over) | mfb(vpf_cargo) },
    { "box",                        'o', c_brown,  '#', c_brown,    60, 100, 400, 0, itm_frame, 0,
        mfb(vpf_over) | mfb(vpf_cargo) },

    { "controls",   '$', c_ltgray,  '$', c_red,     10, 250, 0, 0, itm_vehicle_controls, 3,
        mfb(vpf_internal)  | mfb(vpf_controls) },
//                                                          bonus
    { "muffler",    '/', c_ltgray,  '/', c_ltgray,  10, 150, 40, 0, itm_muffler, 1,
        mfb(vpf_internal)  | mfb(vpf_muffler) },
    { "seatbelt",   ',', c_ltgray,  ',', c_red,     10, 200, 25, 0, itm_rope_6, 1,
        mfb(vpf_internal)  | mfb(vpf_seatbelt) },
    { "solar panel", '#', c_yellow,  'x', c_yellow, 10, 20, 30, 0, itm_solar_panel, 3,
        mfb(vpf_over)  | mfb(vpf_solar_panel) },

    { "mounted M249",         't', c_cyan,    '#', c_cyan,    80, 400, 0, AT_223, itm_m249, 4,
        mfb(vpf_over)  | mfb(vpf_turret) | mfb(vpf_cargo) },
    { "mounted flamethrower", 't', c_dkgray,  '#', c_dkgray,  80, 400, 0, AT_GAS, itm_flamethrower, 5,
        mfb(vpf_over)  | mfb(vpf_turret) },
    { "mounted plasma gun", 't', c_ltblue,    '#', c_ltblue,    80, 400, 0, AT_PLASMA, itm_plasma_rifle, 6,
        mfb(vpf_over)  | mfb(vpf_turret) },

    { "steel plating",     ')', c_ltcyan, ')', c_ltcyan, 100, 1000, 0, 0, itm_steel_plate, 2,
    ...
    { "hard plating",      ')', c_cyan,   ')', c_cyan,   100, 2300, 0, 0, itm_hard_plate, 3,
        mfb(vpf_internal) | mfb(vpf_armor) }
};
