...
void game::drop()
{
	std::list<char> chars = multiInv("Drop item(s):", INV_DROP);
	if (chars.back() == KEY_ESCAPE) {
		add_msg("Never mind.");
		return;
	}
...
{
	return singleInv(title, INV_OTHER);
}

char game::singleInv(std::string title, inv_menus menu)
{
	return allInv(title, false, menu).front();
}

std::list<char> game::multiInv(std::string title, inv_menus menu)
{
	return allInv(title, true, menu);
}

// Display current inventory.
std::list<char> game::allInv(std::string title, bool multiAllowed, inv_menus menu)
{
	bool multiMode = false;//if multipule selection is toogled on.
	std::list<char> selects;//want to maintain order

	bool generalStuffAllowed = true, wornAllowed = true, wieldAllowed = true;
	int weightCostMult = 0, weightCostMultWorn = 0/*and wield*/,
			weightCostAccum = 0;
	int volumeCostMult = 0, volumeCostMultWorn = 0,
			volumeCostAccum = 0;
	int volumeCostMultStore = 0, storageCostAccum = 0;
	switch(menu)
	{
	case INV_DROP:
	{
		weightCostMult = -1;
		volumeCostMult = -1;
		weightCostMultWorn = -1;
		break;
	}
	case INV_UNWEAR:
	{
		generalStuffAllowed = false;
		volumeCostMultWorn = 1;
		volumeCostMultStore = -1;
		break;
	}
	}
	...
	if(multiAllowed)
		mvwprintw(w_inv, 1, 1, "/ toggles mulipule selection");
	...
	do {
		...
		{
			item* itm = &u.i_at(ch);
			bool worn = u.wearing_item(itm) || u.wielding_item(itm);

			if(std::find(selects.begin(), selects.end(), ch)!=selects.end())
			{
				bool canTick = false;
				if(worn && (wornAllowed || !itm->is_armor()) && (wieldAllowed || !itm->is_weap()))
				{
					weightCostAccum -= itm->weight()*weightCostMultWorn;
					volumeCostAccum -= itm->volume()*volumeCostMultWorn;
					if(itm->is_armor())
						storageCostAccum -= (dynamic_cast<it_armor*>(itm->type))->storage*volumeCostMultStore;
					canTick = true;
				}
				else if(generalStuffAllowed && !(itm->is_armor() || itm->is_weap()))
				{
					weightCostAccum -= itm->weight()*weightCostMult;
					volumeCostAccum -= itm->volume()*volumeCostMult;
					canTick = true;
				}
				if(canTick)
					selects.remove(ch);
		...
		if (post_wei >= u.weight_capacity() * .25)
			...
			wprintz(w_inv, c_ltgray, "%d", post_wei);
		...
		if (post_vol > u.volume_capacity() - 2)
			...
			wprintz(w_inv, c_ltgray, "%d", post_vol);
		wprintw(w_inv, "/%d", u.volume_capacity() - 2);
		// Print our weapon
		mvwprintz(w_inv, 2, 40, c_magenta, "WEAPON:");

		std::string flag;
		nc_color c_var = c_white;
#define FIND_ITEM_DEFINITIONS(letter, cond) \
		flag = ""; \
		c_var = c_white; \
		if(cond && multiMode) { \
			if(std::find(selects.begin(), selects.end(), letter)!=selects.end()) { \
				c_var = c_ltgreen; \
				flag = "+"; \
			} else \
			flag = "-"; \
		...
		FIND_ITEM_DEFINITIONS(u.weapon.invlet, u.is_armed() && wieldAllowed);
		if(flag == "")
			mvwprintz(w_inv, 3, 41, c_var, " %s", (u.weapname()+"  ").c_str());
		else
			mvwprintz(w_inv, 3, 41, c_var, "%s", (" "+flag+" "+u.weapname()).c_str());
		if (u.is_armed())
			mvwputch(w_inv, 3, 40, c_var, u.weapon.invlet);
		// Print worn items
		if (u.worn.size() > 0)
			mvwprintz(w_inv, 5, 40, c_magenta, "ITEMS WORN:");
		for (int i = 0; i < u.worn.size(); i++) {
			FIND_ITEM_DEFINITIONS(u.worn[i].invlet, wornAllowed);
			mvwputch (w_inv, 6 + i, 40, c_var, u.worn[i].invlet);
			if(flag == "")
				mvwprintz(w_inv, 6 + i, 41, c_var, " %s", (u.worn[i].tname(this)+"  ").c_str());
			else
				mvwprintz(w_inv, 6 + i, 41, c_var, "%s", (" "+flag+" "+u.worn[i].tname(this)).c_str());
		}
		for (cur_it = start; cur_it < start + maxitems && cur_line < 23; cur_it++) {
			...
			if (cur_it < u.inv.size()) {
				FIND_ITEM_DEFINITIONS(u.inv[cur_it].invlet, generalStuffAllowed);
				mvwputch (w_inv, cur_line, 0, c_var, u.inv[cur_it].invlet);
				if(flag == "")
					mvwprintz(w_inv, cur_line, 1, c_var, " %s", (u.inv[cur_it].tname(this)).c_str());
				else
					mvwprintz(w_inv, cur_line, 1, c_var, " %s", (flag+" "+u.inv[cur_it].tname(this)).c_str());
				if (u.inv[cur_it].charges > 0)
					wprintz(w_inv, c_var, " (%d)  ", u.inv[cur_it].charges);
			}
			cur_line++;
		}
		if (start > 0)
...
void game::takeoff()
{
	std::list<char> chars = multiInv("Take off item(s):", INV_UNWEAR);
	if (chars.front() == KEY_ESCAPE) {
		add_msg("Never mind.");
		return;
	}
	for(std::list<char>::iterator it = chars.begin(); it != chars.end(); it++)
	{
		...
		if (u.takeoff(this, ch))
		{
			...
			add_msg("You take off your %s.", tmp.tname(this).c_str());
		}
		else
