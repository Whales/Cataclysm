...
void player::disp_info(game *g)
{
	...
	#define STAT_STATUS(stat_cur, stat_max, line) \
		status = c_white; \
		if (stat_cur <= 0) \
			status = c_dkgray; \
		else if (stat_cur < stat_max / 2) \
			status = c_red; \
		else if (stat_cur < stat_max) \
			status = c_ltred; \
		else if (stat_cur == stat_max) \
			status = c_white; \
		else if (stat_cur < stat_max * 1.5) \
			status = c_ltgreen; \
		else \
			status = c_green; \
		mvwprintz(w_stats, line, (stat_max < 10 ? 17 : 16), status, "%d", stat_cur);

	STAT_STATUS(str_cur, str_max, 2);
	STAT_STATUS(dex_cur, dex_max, 3);
	STAT_STATUS(int_cur, int_max, 4);
	STAT_STATUS(per_cur, per_max, 5);
...
void player::disp_status(WINDOW *w)
{
	...
	#define STAT_COLOR(stat_cur, stat_max, col_stat) \
		if(stat_cur < stat_max) \
			col_stat = c_red; \
		else if(stat_cur > stat_max) \
			col_stat = c_green; \

	STAT_COLOR(str_cur, str_max, col_str);
	STAT_COLOR(dex_cur, dex_max, col_dex);
	STAT_COLOR(int_cur, int_max, col_int);
	STAT_COLOR(per_cur, per_max, col_per);
	int spd_cur = current_speed();
	STAT_COLOR(spd_cur, 100, col_spd);
...
bool player::wearing_item(char let)
{
	for (int i = 0; i < worn.size(); i++) {
		if (worn[i].invlet == let)
			return true;
...
bool player::wearing_item(item *it)
{
	for (int i = 0; i < worn.size(); i++) {
		if (it == &(worn[i]))
			return true;
...
bool player::wielding_item(char let)
{
	return weapon.invlet == let;
}

bool player::wielding_item(item *it)
{
	return it == &weapon;
}
...
bool player::takeoff(game *g, char let)
{
	if(weapon.invlet == let)
	{
		if(weapon.type->id > num_items)
		{
			g->add_msg("You cannot drop your %s.", weapon.tname(g).c_str());
			return false;
		}
		else if(volume_capacity() > volume_carried() + weapon.volume())
		{
			inv.push_back(weapon);
			weapon = ret_null;
			inv_sorted = false;
			return true;
		}
