...
public:
	...
	char inv(std::string title = "INVENTORY:");//remove later?
	char singleInv(std::string title = "INVENTORY:", inv_menus menu = INV_OTHER);
	std::list<char> multiInv(std::string title = "INVENTORY:", inv_menus menu = INV_OTHER);
...
private:
	...
	std::list<char> allInv(std::string title, bool multiAllowed, inv_menus menu);
